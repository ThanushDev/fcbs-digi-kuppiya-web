...
export default function VideoManagement() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-bold text-gray-900">Videos</h1>
      ...
      <form onSubmit={handleSubmit} className="mb-8 rounded-xl border border-gray-200 bg-white p-5 space-y-4">
        <div className="flex flex-wrap gap-3">
          ...
          <input type="text" placeholder="YouTube Video ID" value={form.fileUrl} onChange={(e) => setForm({ ...form, fileUrl: e.target.value })}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-500">No videos added yet.</div>
      ) : (
        <div className="space-y-3">
          {items.map((item) => (
            <div key={item.id} className="flex items-center justify-between rounded-xl border border-gray-200 bg-white p-4">
              <div className="flex items-center gap-3 min-w-0">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-red-600/10">
              ...
              <div className="flex gap-2 shrink-0">
                {item.fileUrl && (
                  <button onClick={() => window.open(`https://www.youtube.com/embed/${item.fileUrl}`, '_blank')}
                    className="rounded-lg bg-red-600/20 px-4 py-2 text-xs font-semibold text-red-400 hover:bg-red-600/30 transition">Preview</button>
                )}
