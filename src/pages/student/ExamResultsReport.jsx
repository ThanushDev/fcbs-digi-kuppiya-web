...
import { ArrowLeft, Search, Download, FileText, AlertCircle, Loader2, ChevronDown } from 'lucide-react'
...
export default function ExamResultsReport() {
  ...
  const generateCSV = () => {
    ...
    const headers = ['Index No', 'Subject Code', 'Subject Name', 'Grade'];
    ...
    a.href = url;
    a.download = `exam_results_${indexNo}_${batch.replace('/', '-')}.csv`;
    a.click();
  ...
  const printReport = () => {
    ...
    const userName = studentName || `${userData?.firstName || ''} ${userData?.lastName || ''}`.trim() || 'N/A';
    const regNo = userData?.regNumber || 'N/A';
    
    printWindow.document.write(`
      <!DOCTYPE html>
      <html>
      <head>
        <title>Examination Results - ${indexNo}</title>
        <style>
          body { font-family: Arial, sans-serif; margin: 20px; }
          .header { text-align: center; margin-bottom: 30px; border-bottom: 2px solid #333; padding-bottom: 15px; }
          .header h1 { margin: 0; font-size: 24px; color: #1a1a1a; }
          .header p { margin: 5px 0; color: #555; }
          .student-info { display: flex; justify-content: space-between; margin-bottom: 20px; font-size: 13px; }
          ...
          .grade-E, .grade-ab { background-color: #f8d7da; color: #721c24; font-weight: bold; }
      ...
      <body>
        <div class="header">
          <h1>FCBS DIGI KUPPIYA</h1>
          <p>Examination Results Report</p>
          <p>${userDept.toUpperCase()} Department | Batch ${batch} | Semester ${semester}</p>
        </div>
        <div class="student-info">
          <div><strong>Student Name:</strong> ${userName}</div>
          <div><strong>Index No:</strong> ${indexNo}</div>
          <div><strong>Reg No:</strong> ${regNo}</div>
          <div><strong>Department:</strong> ${userDept.toUpperCase()}</div>
        ...
        <table>
          ...
          <tbody>
            ${results.map((r, i) => {
              const gradeClass = r.grade.startsWith('A') ? 'grade-A' : 
                                r.grade.startsWith('B') ? 'grade-B' :
                                r.grade.startsWith('C') ? 'grade-C' :
                                r.grade.startsWith('D') ? 'grade-D' : 'grade-E';
              return `
              <tr>
                <td>${i + 1}</td>
                <td>${r.subjectCode}</td>
                <td>${r.subjectName}</td>
                <td class="${gradeClass}">${r.grade === 'AB' ? 'AB' : r.grade}</td>
              </tr>
              `;
            }).join('')}
          </tbody>
        ...
        <div class="footer">
          <p>Generated on ${new Date().toLocaleDateString()} | FCBS DIGI KUPPIYA Examination System</p>
          <p>Note: Only the highest grade is shown for repeated subjects. 'AB' indicates absent.</p>
        </div>
    ...
    <div className="max-w-3xl mx-auto animate-fade-in">
      ...
      <div className="card p-6 space-y-6">
        <div className="text-center mb-4">
          <h1 className="text-2xl font-bold text-gray-900">Examination Results Report</h1>
          <p className="text-sm text-gray-500 mt-1">Enter your Index Number and Batch to view results</p>
        </div>
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  ...
                  {userDept.toUpperCase()} | Batch {batch} | Semester {semester} | {results.length} subject(s)
                </p>
              </div>
              <div className="flex gap-2">
                ...
                <button
                  onClick={printReport}
                  className="flex items-center gap-2 px-4 py-2 text-sm font-medium text-indigo-700 bg-indigo-50 border border-indigo-200 rounded-lg hover:bg-indigo-100 transition"
                >
